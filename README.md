...
Web builds target **WebGPU with automatic WebGL2 fallback** (graphics APIs set explicitly in Player Settings and every Web build profile): browsers with WebGPU (Chrome/Edge, Safari 26+, recent Firefox) get the modern backend, everything else falls back to WebGL2. Unity 6000.4's Web WebGPU support is still maturing — if a rendering issue ever appears on a WebGPU browser, the rollback is mechanical but multi-file: swap the API order (or drop the `WebGLSupport` entry) in `ProjectSettings.asset` **and** all five Web build profiles, and update `WebGraphicsSettingsTests` to match (it intentionally goes red if the six copies drift). The configured API list and template selection are asserted in CI by `WebGraphicsSettingsTests`. (Three.js is not involved anywhere — Unity generates its own WebAssembly/JS runtime.)
