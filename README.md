...
Web builds target **WebGPU with automatic WebGL2 fallback** (graphics APIs set explicitly in Player Settings and every Web build profile): browsers with WebGPU (Chrome/Edge, Safari 26+, recent Firefox) get the modern backend, everything else falls back to WebGL2. Unity 6000.4's Web WebGPU support is still maturing — if a rendering issue ever appears on a WebGPU browser, reordering the API list back to WebGL2-first is the one-line rollback. The configured API list and template selection are asserted in CI by `WebGraphicsSettingsTests`. (Three.js is not involved anywhere — Unity generates its own WebAssembly/JS runtime.)
