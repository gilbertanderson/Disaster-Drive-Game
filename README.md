...
- Adaptive controls: keyboard, gamepad, and on-screen touch controls (virtual stick bottom-left, pause bottom-right), with controller hints at the top left and a TOUCH CONTROLS toggle under them — on the start screen and in-game; the toggle choice persists via `PlayerPrefs`.
